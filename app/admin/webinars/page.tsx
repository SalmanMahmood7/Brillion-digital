...
import { webinarsService, Webinar, initializeWebinars, resetToSixWebinars, youtubeService } from "@/lib/firebase-services"
...
export default function AdminWebinars() {
  ...
  const fetchWebinars = async () => {
    try {
      ...
      console.log('Admin panel - fetched webinars from database:', webinarsList.length)
      setWebinars(webinarsList)
    } catch (error) {
      console.error('Error fetching webinars from database:', error)
      setError('Failed to load webinars from database. Please check your Firebase connection.')
      setWebinars([]) // Ensure empty array on error
    } finally {
      setLoading(false)
    }
  }

  // Removed handleInitializeWebinars - no more fake data initialization
  ...
  const handleAddWebinar = async () => {
    try {
      ...
      console.log('Creating webinar with real YouTube data...')
      ...
      await webinarsService.create({
        ...
        duration: "0:00", // Will be auto-fetched from YouTube  
      ...
      console.log('Webinar created with real data!')
    } catch (error) {
  ...
  const handleUpdateWebinar = async () => {
    ...
    try {
      // If YouTube ID changed, fetch new real data
      let updateData = {
        ...newWebinar,
        topics: newWebinar.topics.length > 0 ? newWebinar.topics : []
      };
      
      if (newWebinar.youtubeId !== editingWebinar.youtubeId) {
        console.log('YouTube ID changed, fetching new real data...');
        setFetchingYouTubeData(true);
        
        try {
          const youtubeData = await youtubeService.getVideoDetails(newWebinar.youtubeId);
          if (youtubeData) {
            updateData.duration = youtubeData.duration;
            updateData.thumbnail = youtubeData.thumbnail;
          }
        } catch (error) {
          console.warn('Could not fetch new YouTube data:', error);
        }
      }
      
      await webinarsService.update(editingWebinar.id, updateData)
      await fetchWebinars()
  ...
  const getStatusBadge = (status: Webinar["status"]) => {
    const colors = {
      upcoming: "bg-blue-100 text-[#1e3a8a]",
            ...
            <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                  <h4 className="font-medium text-blue-900 mb-2">📊 Real-Time Analytics</h4>
                  <p className="text-sm text-blue-700">
                    <strong>Duration & Views:</strong> Automatically fetched from YouTube API<br/>
                    <strong>View Tracking:</strong> Real views tracked when visitors watch from your website<br/>
                    <strong>Thumbnail:</strong> Auto-generated from YouTube video
                  </p>
                </div>
                ...
                {/* Thumbnail is auto-generated from YouTube - no manual input needed */}
          ...
          <CardContent>
            {loading ? (
              <div className="flex justify-center items-center h-64">
                <BrillionLoader size="lg" theme="orange" showText={true} text="Loading webinars from database..." />
              </div>
            ) : error ? (
              <div className="text-center py-16">
                <Video className="w-16 h-16 mx-auto mb-4 text-red-300" />
                <h3 className="text-xl font-semibold text-red-600 mb-2">Database Connection Error</h3>
              ...
              <div className="text-center py-16">
                ...
                <h3 className="text-xl font-semibold text-gray-600 mb-2">No webinars found in database</h3>
                <p className="text-gray-500 mb-4">
                  Start by adding your first webinar to the real-time database. No sample data needed.
                </p>
                <Button 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewWebinar({...newWebinar, views: parseInt(e.target.value) || 0})}
