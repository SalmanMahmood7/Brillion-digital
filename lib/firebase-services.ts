...
// YouTube API service for real video data
export const youtubeService = {
  // Get video details from YouTube
  async getVideoDetails(youtubeId: string): Promise<{
    duration: string;
    thumbnail: string;
    title?: string;
    viewCount?: number;
    ...
    try {
      // This is a public API that doesn't require API key for basic info
      const response = await fetch(`https://www.youtube.com/oembed?url=https://www.youtube.com/watch?v=${youtubeId}&format=json`);
      
      if (!response.ok) {
        console.warn('YouTube oEmbed API failed, using defaults');
        return {
          duration: "0:00",
      ...
      // Generate thumbnail URL (always available)
      const thumbnail = `https://img.youtube.com/vi/${youtubeId}/maxresdefault.jpg`;
      
      // For duration, we need to make another call or use a different approach
      // Let's try to get it from YouTube's public API (no key needed for public videos)
      let duration = "0:00";
      
      try {
        // Try to get more details (this might not work without API key)
        const detailsResponse = await fetch(`https://www.googleapis.com/youtube/v3/videos?id=${youtubeId}&part=contentDetails&key=YOUR_API_KEY`);
        if (detailsResponse.ok) {
          const detailsData = await detailsResponse.json();
          if (detailsData.items && detailsData.items[0]) {
            duration = this.convertISO8601ToTime(detailsData.items[0].contentDetails.duration);
          }
        }
      } catch (error) {
        console.log('YouTube API key not configured, using default duration');
      }
      ...
      return {
        ...
        thumbnail,
        title: data.title,
        viewCount: 0 // We'll track our own views
      };
    } catch (error) {
  ...
  convertISO8601ToTime(duration: string): string {
...
export interface Webinar {
  ...
  description: string;
  category: string;
  duration: string; // Auto-fetched from YouTube API (e.g. "34:52")
  views: number; // Real view count from our analytics, starts at 0
  youtubeViews?: number; // YouTube's public view count (if available)
  date: string;
  youtubeId: string;
  speaker: string;
  topics: string[];
  ...
  totalWatched?: number; // How many people watched from our site
  averageWatchTime?: number; // Average watch time in seconds
  lastWatched?: Timestamp; // When someone last watched
}
...
export const webinarsService = {
  ...
  async create(webinar: Omit<Webinar, 'id'>): Promise<string> {
    try {
      // Auto-fetch real YouTube data
      let duration = "0:00";
      let thumbnail = webinar.thumbnail || "";
      
      if (webinar.youtubeId) {
        try {
          console.log('Fetching real YouTube data for:', webinar.youtubeId);
          const youtubeData = await youtubeService.getVideoDetails(webinar.youtubeId);
          if (youtubeData) {
            duration = youtubeData.duration;
            thumbnail = youtubeData.thumbnail;
            console.log('Got real duration:', duration);
          }
        } catch (error) {
          console.warn('Could not fetch YouTube data, using defaults:', error);
        }
      }
      
      const docRef = await addDoc(collection(db, 'webinars'), {
        ...webinar,
        duration, // Real duration from YouTube
        views: 0, // Start with 0 real views
        thumbnail, // Real thumbnail from YouTube
        totalWatched: 0,
        averageWatchTime: 0,
        registeredCount: 0,
        createdAt: Timestamp.now(),
      ...
      console.log('Webinar created with real YouTube data. Duration:', duration, 'Views: 0');
      return docRef.id;
    } catch (error) {
  ...
  // Track real view when someone watches a webinar
  async trackView(id: string, watchTimeSeconds?: number): Promise<void> {
    try {
      const webinarRef = doc(db, 'webinars', id);
      const webinarDoc = await getDoc(webinarRef);
      
      if (webinarDoc.exists()) {
        const webinar = webinarDoc.data() as Webinar;
        const currentViews = webinar.views || 0;
        const currentTotalWatched = webinar.totalWatched || 0;
        const currentAverageWatchTime = webinar.averageWatchTime || 0;
        
        // Calculate new average watch time if provided
        let newAverageWatchTime = currentAverageWatchTime;
        if (watchTimeSeconds !== undefined && currentTotalWatched > 0) {
          newAverageWatchTime = ((currentAverageWatchTime * currentTotalWatched) + watchTimeSeconds) / (currentTotalWatched + 1);
        } else if (watchTimeSeconds !== undefined && currentTotalWatched === 0) {
          newAverageWatchTime = watchTimeSeconds;
        }
        
        await updateDoc(webinarRef, {
          views: currentViews + 1,
          totalWatched: currentTotalWatched + 1,
          averageWatchTime: newAverageWatchTime,
          lastWatched: Timestamp.now(),
          updatedAt: Timestamp.now()
        });
        
        console.log(`Real webinar view tracked! New count: ${currentViews + 1}`);
      }
    } catch (error) {
      console.error('Error tracking webinar view:', error);
      // Don't throw error - view tracking shouldn't break the user experience
    }
  },
    ...
    try {
      ...
      if (webinarDoc.exists()) {
        ...
        return {
          ...
          totalWatched: webinar.totalWatched || 0,
          averageWatchTime: webinar.averageWatchTime || 0,
          lastWatched: webinar.lastWatched?.toDate()
        };
      }
      return null;
    } catch (error) {
...
// Clear all webinars (no re-initialization with fake data)
export async function resetToSixWebinars() {
  try {
    // Clear everything
    await clearAllWebinars();
    
    console.log('✅ All webinars cleared - Real-time database is now empty');
    console.log('ℹ️ Add webinars manually through the admin panel');
  } catch (error) {
    console.error('❌ Error clearing webinars:', error);
    throw error;
  }
...
// Initialize empty webinars collection (no fake data)
export async function initializeWebinars() {
  console.log('No sample data initialization - Real-time database only');
  // This function is kept for compatibility but does nothing
  // Webinars should be added manually through the admin panel
}
...
export interface User {
  ...
  lastLogin?: Timestamp;
  updatedAt: Timestamp;
  signInMethod?: string; // 'email', 'google', etc.
...
export const usersService = {
  ...
  async create(userData: Omit<User, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    try {
      ...
      console.log('User created in database:', docRef.id);
      return docRef.id;
    } catch (error) {
  ...
  // Update user by UID
  async updateByUid(uid: string, updates: Partial<User>): Promise<void> {
    try {
  ...
  // Record user sign-in
  async recordSignIn(uid: string, signInMethod?: string): Promise<void> {
    try {
      const user = await this.getByUid(uid);
      if (user && user.id) {
        ...
        await this.update(user.id, {
          ...
          signInMethod: signInMethod || user.signInMethod
        });
      }
    } catch (error) {
  ...
  // Get recent signups
  async getRecentSignups(limitCount: number = 10): Promise<User[]> {
    try {
      const usersRef = collection(db, 'users');
      const q = query(
        usersRef,
  ...
  // Create or update user (for sign-up/sign-in)
  async createOrUpdate(authUser: {
    uid: string;
    email: string;
    displayName: string | null;
    photoURL?: string | null;
  }, signInMethod?: string): Promise<User> {
    try {
      const existingUser = await this.getByUid(authUser.uid);
      
      if (existingUser && existingUser.id) {
        // Update existing user
        await this.recordSignIn(authUser.uid, signInMethod);
        return { ...existingUser, lastLogin: Timestamp.now() };
      } else {
        // Create new user
        ...
        const userId = await this.create(newUserData);
        return {
          id: userId,
          ...newUserData,
          createdAt: Timestamp.now(),
